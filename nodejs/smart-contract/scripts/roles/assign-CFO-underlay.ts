...
async function main(): Promise<void> {
  ...
  const target = env.CFO_ADDRESS_UNDERLAY || env.CFO_ADDRESS;
  const currentCfo = await contract.financialOfficer();
  ...
  console.log(
    chalk.cyan(
      `Assigning financial officer on the underlay (${chalk.bold(networkName)}) (contract ${contractAddress})...`,
    ),
  );
